...
const pdfIcon = workspaceIcon("pdf"), wordIcon = workspaceIcon("word"), spreadsheetIcon = workspaceIcon("spreadsheet"), codeIcon = workspaceIcon("code"), imageIcon = workspaceIcon("image"), audioIcon = workspaceIcon("audio"), markdownIcon = workspaceIcon("markdown"), documentIcon = workspaceIcon("document"), applicationIcon = workspaceIcon("application");
...
function workspaceIconSource(item: LibraryItem | WorkspaceResource) { const extension = item.target.match(/\.([^.\\/]+)$/)?.[1]?.toLowerCase() || ""; if (/^(doc|docx|odt|rtf)$/i.test(extension)) return wordIcon; if (/^(xls|xlsx|csv|ods)$/i.test(extension)) return spreadsheetIcon; if (extension === "pdf") return pdfIcon; if (/^(md|mdx)$/i.test(extension)) return markdownIcon; if (/^(js|ts|tsx|py|json|html|css|yml|yaml|xml)$/i.test(extension)) return codeIcon; if (/^(png|jpe?g|gif|webp|svg|tiff?)$/i.test(extension)) return imageIcon; if (/^(mp3|wav|m4a|flac|ogg)$/i.test(extension)) return audioIcon; if (/^(exe|com|bat|cmd)$/i.test(extension)) return applicationIcon; return documentIcon; }
