...
import type { Group, ImportedResource, ItemKind, LibraryItem, LibraryLayout, LibraryState, PanelId, SortMode, Workspace, WorkspaceResource } from "./types";
...
const displayName = (item: LibraryItem | WorkspaceResource) => {
  const extension = item.kind === "file" ? (item.target.match(/(\.[^\\/.]+)$/)?.[1] || "") : "";
...
export default function App() {
  ...
  const workspaceItems = activeWorkspace ? [...activeWorkspace.resources, ...activeWorkspace.itemIds.map((itemId) => state.items.find((item) => item.id === itemId)).filter(Boolean) as LibraryItem[]] : [];
...
function PreviewList({ items, onLaunch, collapsed, empty }: { items: Array<LibraryItem | WorkspaceResource>; onLaunch(item: LibraryItem | WorkspaceResource): void; collapsed: boolean; empty: string }) {
...
function WorkspacePreview({ items, onLaunch, onAddFolder, onAddFile, collapsed }: { items: Array<LibraryItem | WorkspaceResource>; onLaunch(item: LibraryItem | WorkspaceResource): void; onAddFolder(): void; onAddFile(): void; collapsed: boolean }) {
  return <div className="pt-3"><div className="flex flex-wrap gap-2"><button className="control compact" onClick={onAddFolder}><FolderOpen />Add folder</button><button className="control compact" onClick={onAddFile}><FilePlus2 />Add file</button></div><PreviewList items={items} onLaunch={onLaunch} collapsed={collapsed} empty="Add folders or files for this workspace. They stay here rather than becoming standalone library items." /></div>;
}
