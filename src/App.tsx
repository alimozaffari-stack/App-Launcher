...
function WorkspacePreview({ items, onLaunch, onAddFolder, onAddFile, collapsed }: { items: Array<LibraryItem | WorkspaceResource>; onLaunch(item: LibraryItem | WorkspaceResource): void; onAddFolder(): void; onAddFile(): void; collapsed: boolean }) {
  return <div className="pt-3"><div className="flex flex-wrap gap-2"><button className="control compact" onClick={onAddFolder}><FolderOpen />Add folder</button><button className="control compact" onClick={onAddFile}><FilePlus2 />Add file</button></div><PreviewList items={items} onLaunch={onLaunch} collapsed={collapsed} showExtensions empty="Add folders or files for this workspace. They stay here rather than becoming standalone library items." /></div>;
}
