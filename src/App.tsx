...
function workspaceIconSource(item: LibraryItem | WorkspaceResource) { const extension = item.target.match(/\.([^.\\/]+)$/)?.[1]?.toLowerCase() || ""; const descriptor = `${item.name} ${item.target}`.toLowerCase(); if (extension === "gdoc" || /\bgoogle[ _-]?doc/.test(descriptor)) return googleDocIcon; if (/^(doc|docx|odt|rtf)$/i.test(extension) || /\b(word|msword)\b/.test(descriptor)) return wordIcon; if (/^(xls|xlsx|csv|ods)$/i.test(extension) || /\b(excel|spreadsheet)\b/.test(descriptor)) return spreadsheetIcon; if (extension === "pdf") return pdfIcon; if (/^(md|mdx)$/i.test(extension) || /\bmarkdown\b/.test(descriptor)) return markdownIcon; if (/^(js|ts|tsx|py|json|html|css|yml|yaml|xml)$/i.test(extension) || /\b(code|script)\b/.test(descriptor)) return codeIcon; if (/^(png|jpe?g|gif|webp|svg|tiff?)$/i.test(extension) || /\b(image|photo|picture)\b/.test(descriptor)) return imageIcon; if (/^(mp3|wav|m4a|flac|ogg)$/i.test(extension) || /\b(audio|sound|music)\b/.test(descriptor)) return audioIcon; if (/^(mp4|mov|mkv|avi|webm)$/i.test(extension) || /\b(video|movie)\b/.test(descriptor)) return movieIcon; if (extension === "com") return comIcon; if (/^(exe|bat|cmd)$/i.test(extension)) return utilityIcon; return documentIcon; }
